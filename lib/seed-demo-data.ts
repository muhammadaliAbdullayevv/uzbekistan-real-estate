...
/**
 * Creates/refreshes the sample listings + demo user used for testing.
 * Safe to re-run: deletions are scoped only to its own known sample-owner
 * emails and the demo account, never a blanket wipe of real user data.
 */
export async function seedDemoData(prisma: PrismaClient) {
  ...
  return {
    ...
    approvedListings: approvedListingIds.length
  };
}
